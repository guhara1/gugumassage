...
function regionPage([slug, ko, en, context, customer]) {
  ...
  const body = `<main>
    <section class="sub-hero">
      ...
      <p>${ko} 출장마사지는 고객이 원하는 장소에서 편안하게 관리를 받을 수 있도록 안내하는 방문형 마사지 서비스입니다. 스웨디시, 아로마테라피, 딥티슈, 타이마사지, 스포츠마사지, 림프마사지 등 고객의 컨디션과 목적에 맞는 다양한 관리 프로그램을 제공합니다.</p>
      <p>${detail.localNote}</p>
      <a class="button solid" href="tel:${phone.replaceAll("-", "")}">${ko} 예약 문의</a>
    </section>
    <article class="long-copy area-detail">
      <h2>${ko} 출장마사지 이용 안내</h2>
      <p>${ko} 출장마사지는 바쁜 일정으로 매장 방문이 어려운 고객, 퇴근 후 편안한 휴식을 원하는 고객, 숙소나 자택에서 조용히 관리를 받고 싶은 고객에게 적합합니다. ${context}에서 이용하는 경우 이동 시간, 주차, 출입 방식, 엘리베이터 이용 가능 여부처럼 작은 조건이 예약 만족도에 큰 영향을 줍니다.</p>
      <p>강한 압보다는 편안한 릴랙스를 원하는 경우 스웨디시나 아로마테라피를, 뭉친 부위를 집중적으로 풀고 싶은 경우 딥티슈나 스포츠마사지를 선택할 수 있습니다. 몸이 뻣뻣하거나 스트레칭 중심의 관리를 원한다면 타이마사지도 좋은 선택이 될 수 있습니다.</p>
      <h2>${ko} 출장 가능 지역</h2>
      ...
      <p>일부 지역은 시간대, 교통 상황, 예약 현황에 따라 방문 가능 시간이 달라질 수 있습니다. ${detail.feeNote}</p>
      <h2>${ko}에서 이용 가능한 마사지 서비스</h2>
      <div class="service-summary">${services.map(([serviceSlug, name, desc, intent]) => `<section><h3><a href="${serviceHref(serviceSlug)}">${name}</a></h3><p>${desc}. ${intent}에게 적합합니다.</p></section>`).join("")}</div>
      <h2>예약 전 확인사항</h2>
      <ul>
        <li>이용 지역과 상세 주소</li>
        <li>희망 날짜와 시간</li>
        <li>원하는 마사지 종류와 관리 시간</li>
        <li>엘리베이터, 주차, 출입 방법 등 방문 안내</li>
        <li>추가 출장비 발생 여부와 결제 방식</li>
      </ul>
      <p>주소와 시간을 정확히 알려주시면 ${ko} 지역 내 방문 가능 여부를 더 빠르게 안내받을 수 있습니다.</p>
      <h2>${ko} 출장마사지 이용 절차</h2>
      <ol>
        <li>원하는 지역과 시간을 기준으로 예약 문의를 남깁니다.</li>
        <li>상담을 통해 마사지 종류와 관리 시간을 선택합니다.</li>
        <li>방문 가능 여부와 요금을 확인합니다.</li>
        <li>예약이 확정되면 안내된 시간에 관리사가 방문합니다.</li>
        <li>고객 컨디션과 원하는 압을 확인한 뒤 관리가 진행됩니다.</li>
        <li>관리 후 마무리 안내와 다음 이용 시 참고사항을 전달합니다.</li>
      </ol>
      <h2>이용 전 준비사항</h2>
      <p>편안한 관리를 위해 관리받을 공간을 미리 확보해 주세요. 과식이나 음주 직후 이용은 피하는 것이 좋으며, 불편한 부위나 원하는 압 강도는 관리 전 미리 알려주시면 도움이 됩니다.</p>
      <p>피부가 민감하거나 특정 오일 사용이 부담스러운 경우 예약 단계에서 미리 안내해 주세요. ${brand}는 질병 치료, 완치, 의학적 회복을 보장하지 않는 건전한 웰니스 관리 안내를 기준으로 운영합니다.</p>
      <h2>자주 묻는 질문</h2>
      <div class="faq service-faq">${detail.faq.map(([q, a]) => `<details><summary>${q}</summary><p>${a}</p></details>`).join("")}</div>
      <h2>예약 문의</h2>
      <p>${ko} 출장마사지 예약은 지역, 희망 시간, 원하는 마사지 종류를 함께 남겨주시면 빠르게 안내해 드립니다. 고객의 컨디션과 목적에 맞는 서비스를 선택할 수 있도록 상담을 통해 자세히 도와드립니다.</p>
    </article>
